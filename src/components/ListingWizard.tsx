...
export default function ListingWizard({ product, onClose }: ListingWizardProps) {
  ...
  const generateWithAI = async () => {
    if(!condition && !era && !style) {
        if(!confirm("You haven't selected any details (Condition, Era, etc). Generate anyway?")) return;
    }
    ...
    try {
      const res = await fetch("/api/ai/generate", {
        ...
        body: JSON.stringify({
            ...
            price: sellingPrice,
            imageUrl: product.imageUrl, // Sending image for vision!
            preferences: { condition, era, gender, style }
        }),
      });
          ...
          {step === "options" ? (
                ...
                <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                    ...
                    <div className="flex gap-4 items-end">
                        ...
                        <div className="flex-1 text-right">
                             ...
                             <p className={`text-xl font-mono font-bold ${profit && profit > 0 ? 'text-green-400' : 'text-gray-600'}`}>
                                {profit ? `$${profit}` : "--"}
                             </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {["Brand New w/ Tags", "Like New", "Vintage Used", "Thrifted"].map(opt => (
                                    <label key={opt} className="flex items-center gap-2 cursor-pointer group">
                                        <input type="radio" name="cond" checked={condition === opt} onChange={() => setCondition(opt)} className="accent-purple-500" />
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {["Modern", "Y2K (2000s)", "90s Grunge", "80s Retro"].map(opt => (
                                    <label key={opt} className="flex items-center gap-2 cursor-pointer group">
                                        <input type="radio" name="era" checked={era === opt} onChange={() => setEra(opt)} className="accent-purple-500" />
                                        <span className="text-sm text-gray-300 group-hover:text-white">{opt}</span>
                                    </label>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="Casual">Basic / Casual</option>
                            </select>
                        </div>

                        {/* Gender */}
                        <div>
                            <label className="text-xs text-gray-400 uppercase font-bold mb-2 block">Target Gender</label>
                            <div className="flex gap-4">
                                {["Womens", "Mens", "Unisex"].map(opt => (
                                    <label key={opt} className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="gen" checked={gender === opt} onChange={() => setGender(opt)} className="accent-purple-500" />
             ...
             <div className="space-y-4 h-full flex flex-col animate-in fade-in slide-in-from-right-4">
                ...
                <textarea 
                    ...
                    className="w-full flex-1 bg-black border border-gray-700 rounded-xl p-4 text-sm text-gray-300 focus:outline-none focus:ring-1 focus:ring-purple-500 resize-none leading-relaxed font-mono"
                />

                <div className="flex gap-3 pt-2">
                    <button 
                        onClick={() => saveToDb(generatedDesc)}
                        className="px-6 bg-gray-800 hover:bg-gray-700 text-white font-bold py-3 rounded-xl transition flex items-center gap-2"
                    >
                    ...
                    <button 
                        ...
                        className="flex-1 bg-white text-black font-bold py-3 rounded-xl hover:bg-gray-200 transition flex justify-center items-center gap-2"
                    >
                        {copied ? <CheckCircle2 size={18} className="text-green-600" /> : <Copy size={18} />}
                        {copied ? "Copied!" : "Copy to Depop"}
                    </button>
                </div>
             </div>
          )}
